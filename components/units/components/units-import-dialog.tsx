...
export function UnitsImportDialog({ open, onOpenChange }: UnitsImportDialogProps) {
  ...
  const handleImport = async () => {
    ...
    setIsLoading(true)
    try {
      const result = await importUnits(files[0])
      toast.success(
        `Successfully imported ${result.imported} units${
      ...
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              Upload a CSV file to import units. Make sure the file follows the required format.
            </DialogDescription>
          </DialogHeader>
  ...
  return (
    <Drawer open={open} onOpenChange={onOpenChange}>
      <DrawerContent>
        <DrawerHeader>
          <DrawerTitle>Import Units</DrawerTitle>
          <DrawerDescription>
            Upload a CSV file to import units. Make sure the file follows the required format.
          </DrawerDescription>
        </DrawerHeader>
        <div className="px-4 space-y-4">
          ...
          <div className="space-y-2">
            ...
            >
              <FileUploadDropzone>
                <p className="text-sm text-muted-foreground">Drag and drop a CSV/Excel file here, or click to browse</p>
              </FileUploadDropzone>
              <FileUploadList>
                {files.map((file, index) => (
                  <FileUploadItem key={index} value={file}>
