...
import { bulkDeleteUnits } from "../data/units"

type UnitMultiDeleteDialogProps<TData> = {
  open: boolean
...
export function UnitsMultiDeleteDialog<TData>({ open, onOpenChange, table }: UnitMultiDeleteDialogProps<TData>) {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      toast.success(`Deleted ${selectedRows.length} ${selectedRows.length > 1 ? "units" : "unit"}`)
    ...
    <ConfirmDialog
      ...
      desc={
        <div className="space-y-4">
          <p className="mb-2">
            Are you sure you want to delete the selected units? <br />
